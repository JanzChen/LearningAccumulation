...
this.jsviews || this.jQuery && jQuery.views || (function (window, undefined) {
    ...
    tags({
        ...
        "stringInterrupt": function (Languages, a) { //{{format induName 13 /}}   截取字符串
            if(Languages){
                return Languages.length > a ? Languages.substr(0, a) + "..." : Languages;
            }
            return "";

        },
        "stringInterruptDate": function (Languages, a) { //{{format induName 13 /}}   截取字符串
            if(Languages){
                return Languages.length > a ? Languages.substr(0, a): Languages;
            }
            return "";
        },
        "formatMoney":function (Languages) { //{{format induName 13 /}}   截取字符串
            if(Languages){
                return Languages.toFixed(2);
            }
            return "";
        }
    ...
    converters({
        ...
        dateIntercept: function (text) {
            //date   2013-03-01 00:00:00  --  2013-03-01
            return text!=null?text.substr(0, 10):"";
        },
        //2013-03-01 00:00:00==> 2013年03月01日
        ds_dateInit:function(text){
             if(text!=null){
               return text.substr(0, 4)+"年"+text.substr(5, 2)+"月"+text.substr(8, 2)+"日";
             }
            return "";
        },

        ds_dateIntercept: function (text) {
            //date   2013-03-01 00:00:00  --  2013-03-01 00:00
            return text!=null?text.substr(0, 16):"";
        },
        //2013-03-01 00:00:00==> 00:00:00
        ds_time:function(text){
            if(text!=null){
                return text.substr(11, 8);
            }
            return "";
        },
        //2013-03-01 00:00:00==>03月01日 00:00
        wyp_dateInit:function(text){
            if(text!=null){
                return text.substr(5, 2)+"月"+text.substr(8, 2)+"日"+text.substr(11,5);
            }
            return "";
        },
        zf_dateInit:function(text){
            return text!=null?text.substr(2,5):"";
        },
        textWrap:function(oriText,wrapText,pre,suffix){
            if(oriText ==null){
                return "";
            }
            oriText.replace(wrapText,pre+wrapText+suffix);
            return oriText;
        },
        timerShaft:function (time){
            var date = new Date();
            var str = '';
            var t = time;
            var YTime = (time.replace("-", "/")).replace("-", "/");
            var DTime = new Date(YTime);
            var HTime = time.substring(time.indexOf(' '),time.lastIndexOf(":"));
            var OTime = (date.getTime() - new Date(date.getFullYear() + "/" + (date.getMonth() + 1) + "/" + date.getDate() + " 00:00:00").getTime()) / 1000;
            var time = (date.getTime() - DTime.getTime()) / 1000;
            if (time <= OTime) {
                if (time < 60) {
                    str = '刚刚';
                } else if (time < 60 * 60) {
                ...
                if ((time - OTime) <= 60 * 60 * 24) {
                    str = '昨天 ' + HTime;
                } else if ((time - OTime) <= 60 * 60 * 24 * 2) {
                    str = '前天 ' + HTime;
                } else {
